...
static void motor_on_off(int on)
{
		...
		if (vreg_msm_vibrator == NULL) {
			...
			printk("Regualtor get successfully done by amit Jain");
		}
	if (on) {

		if (!enabled) {
			enabled = 1;
			printk("vib regualtor is enabled in board file by amit JAIN \n");
		...
		if (enabled) {
			enabled = 0;
			printk("vib regualtor is disabled in board file by amit JAIN  \n");
...
#ifdef CONFIG_SAMSUNG_JACK
#if defined (CONFIG_MACH_CANE_ATT) || defined (CONFIG_MACH_CANE_TMO)
...
static struct platform_device *cane_devices[] __initdata = {
	&msm8960_device_otg,
...
void __init msm8930_cane_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#endif
	platform_add_devices(cane_devices, ARRAY_SIZE(cane_devices));
#ifdef CONFIG_MSM_CAMERA
	msm8930_init_cam();
#endif
	msm8930_init_mmc();
