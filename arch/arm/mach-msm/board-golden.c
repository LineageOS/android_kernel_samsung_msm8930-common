...
#define MSM_ION_MM_SIZE            0x4C00000 /* Need to be multiple of 64K */
...
#if defined(CONFIG_CHARGER_SMB347) || defined(CONFIG_CHARGER_SMB358)
...
#if defined(CONFIG_INPUT_YAS_SENSORS)
#if  defined(CONFIG_INPUT_MPU6500)
static struct i2c_board_info sns_i2c_board_info_rev02[] __initdata = {
	{
		I2C_BOARD_INFO("mpu6500_input", 0x62), /*dummy address*/
		.platform_data = &mpu6k_pdata,
		.irq = MSM_GPIO_TO_INT(GPIO_ACC_INT_N),
	},
	{
...
#if defined(CONFIG_INPUT_YAS_SENSORS) || defined(CONFIG_OPTICAL_GP2AP020A00F) \
	|| defined(CONFIG_PROXIMITY_SENSOR)  || defined(CONFIG_INPUT_MPU6500)
...
static void opt_led_on(bool on)
{
	...
	if (vsensor_3p0 == NULL) {
		vsensor_3p0 = regulator_get(NULL, "8917_l30");
...
static struct platform_device *golden_devices[] __initdata = {
	&msm8960_device_otg,
...
#if defined(CONFIG_INPUT_MPU6500)
static struct i2c_registry sns_i2c_devices_rev02[] __initdata = {
	{
		MSM_SNS_I2C_BUS_ID,
...
#if  defined(CONFIG_INPUT_MPU6500)
	for (i = 0; i < ARRAY_SIZE(sns_i2c_devices_rev02); ++i) {
		i2c_register_board_info(sns_i2c_devices_rev02[i].bus,
		sns_i2c_devices_rev02[i].info,
		sns_i2c_devices_rev02[i].len);
	}
#endif
#ifdef CONFIG_MSM_CAMERA
...
void __init msm8930_golden_init(void)
{
#ifdef CONFIG_SEC_DEBUG
	sec_debug_init();
#endif
...
#endif
	platform_add_devices(golden_devices, ARRAY_SIZE(golden_devices));
#ifdef CONFIG_MSM_CAMERA
	msm8930_init_cam();
#endif
	msm8930_init_mmc();
