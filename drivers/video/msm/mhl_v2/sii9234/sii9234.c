...
u8 sii9234_mhl_get_version(void)
{
	...
	return sii9234->devcap.mhl_ver;
}
EXPORT_SYMBOL(sii9234_mhl_get_version);
...
static int sii9234_cbus_init(struct sii9234_data *sii9234)
{
	...
	if (ret < 0)
		goto i2c_error_exit;
#ifdef CONFIG_MACH_M2_REFRESHSPR
	ret = cbus_write_reg(sii9234, 0x81, 0x13);/*mhl version 1.3*/
#else
	ret = cbus_write_reg(sii9234, 0x81, 0x12);/*mhl version 1.2*/
#endif
	if (ret < 0)
		goto i2c_error_exit;
...
static void goto_d3(struct work_struct *work)
{
	...
	if (sii9234->pdata->power_state == false) {
		pr_info("goto d3 : mhl power_state is false (exit recommended)\n");
		goto exit;
	}
	...
	list_for_each_entry_safe(p_msc_pkt, scratch,
		...
		case CBUS_READ_DEVCAP:
			...
			case DEVCAP_MHL_VERSION:
				...
				pr_info("sii9234: MHL_VERSION: %X\n", value);
				break;
			case DEVCAP_DEV_CAT:
				sii9234->plim = (value >> 5) & 0x03;
				pr_info("sii9234: PLIM : %d\n", sii9234->plim);
...
static irqreturn_t sii9234_irq_thread(int irq, void *data)
{
	...
	if (intr4 & RGND_READY_INT) {
		...
		if (sii9234->rgnd != RGND_1K) {
			...
			sii9234->state = STATE_RGND_NOT_1K;
			goto err_exit;
		}
	}
	...
	if (intr1 & RSEN_CHANGE_INT) {
		...
		if (sii9234->rgnd != RGND_1K) {
			pr_err("[ERROR] sii9234: Err RSEN_HIGH without RGND_1K"
						 "rgnd=%d\n", sii9234->rgnd);
			goto err_exit2;
		}
		ret = mhl_tx_read_reg(sii9234, MHL_TX_SYSSTAT_REG, &value);
...
#endif
	wakeup_source_init(&sii9234->mhl_ws, "mhl_ws");
