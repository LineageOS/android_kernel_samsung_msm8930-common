					 ...
					 int adc)
{
	...
	case ADC_JIG_USB_ON:
		...
		path = AP_USB_MODE;
		break;
	/*Set the cable as USB Data for LG 200KOhm cable*/
	case ADC_CEA936ATYPE1_CHG:
	case ADC_CEA936ATYPE2_CHG:
	case ADC_OPEN:
					  ...
					  int adc, int chgtyp)
{
	...
	case ADC_SMARTDOCK:
		...
		if (mdata->smartdock_cb)
			mdata->smartdock_cb(MAX77693_MUIC_ATTACHED, info->cable_type);
		break;
	default:
...
static void max77693_muic_detach_smart_dock(struct max77693_muic_info *info)
{
	...
	if (mdata->smartdock_cb)
		mdata->smartdock_cb(false,info->cable_type);
				       ...
				       u8 status1, u8 status2, int irq)
{
	...
	} else if (info->cable_type == CABLE_TYPE_CARDOCK_MUIC && adc != ADC_CARDOCK) {
	...
	} else if(info->cable_type == CABLE_TYPE_SMARTDOCK_MUIC || 
		info->cable_type == CABLE_TYPE_SMARTDOCK_TA_MUIC || 
		info->cable_type == CABLE_TYPE_SMARTDOCK_USB_MUIC) {
...
/*Removed setting the cable TA for LG data cable*/
/*Add the LG cable to be checked as TA here*/
#ifdef CONFIG_USBHUB_USB3803
					...
					u8 status1, u8 status2)
{
	...
	dev_info(info->dev, "adclow:%x adc1k:%x\n",
		adclow, adc1k);
	if (adclow && adc1k) {
	...
	switch (adc) {
	case ADC_CHARGING_CABLE:
		pr_info("%s:%s Charging cable connected\n", DEV_NAME, __func__);
		break;
	case ADC_GND:
		if (!adclow) {
	...
	case (ADC_AUDIODOCK + 1) ... (ADC_CHARGING_CABLE - 1):
		dev_warn(info->dev, "%s: unsupported ADC(0x%02x)\n",
				__func__, adc);
		intr = INT_DETACH;
		break;
...
#if defined(CONFIG_MACH_MELIUS_SKT) || defined(CONFIG_MACH_MELIUS_KTT) || \
			defined(CONFIG_MACH_MELIUS_LGT)  || defined(CONFIG_MACH_MELIUS_ATT) || \
			defined(CONFIG_MACH_MELIUS_MTR) || defined(CONFIG_MACH_MELIUS_EUR_OPEN) || \
			defined(CONFIG_MACH_MELIUS_EUR_LTE)
	if (gInfo->cable_type != CABLE_TYPE_DESKDOCK_MUIC)
		return 0;
#endif
...
static int __devinit max77693_muic_probe(struct platform_device *pdev)
{
	...
	schedule_delayed_work(&info->dock_usb_work, msecs_to_jiffies(27000));
#else
