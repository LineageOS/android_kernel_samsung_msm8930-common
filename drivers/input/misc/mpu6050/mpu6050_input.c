...
static int mpu6050_input_resume_gyro(struct mpu6050_input_data *data)
{
	...
	regs[0] = MPUREG_FULLSCALE_VALUE(MPU_FS_500DPS);
	CHECK_RESULT(mpu6050_i2c_write_single_reg
		(data->client, MPUREG_GYRO_CONFIG, regs[0]));

	regs[0] = MPU_FILTER_42HZ | 0x18;
	CHECK_RESULT(mpu6050_i2c_write_single_reg
...
static ssize_t mpu6050_input_gyro_selftest_show(struct device *dev,
					struct device_attribute *attr,
						char *buf)
{
	struct mpu6050_input_data *data = gb_mpu_data;
	int scaled_gyro_bias[3] = {0};
	int scaled_gyro_rms[3] = {0};
	int packet_count[3] = {0};
	int ratio[3] = {0};
	...
	hw_result = mpu6050_gyro_hw_self_check(data->client,
					ratio);
					...
					const struct i2c_device_id *id)
{
	...
	struct mpu6k_input_platform_data *mpu_pdata =
